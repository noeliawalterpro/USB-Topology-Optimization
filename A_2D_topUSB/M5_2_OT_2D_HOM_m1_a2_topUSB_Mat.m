...
%% START ITERATION
while    (loop < maxloop   ) && 1==(abs(cant-c)/cant>10*tolx || abs(xPor-PorObj)>tolx || changea>0.01 || changeb>0.01 )
    loop= loop + 1;
    cant=c;  
    %% FE-ANALYSIS
    eval(nfem)
    ...
    for i= 1:size(F,2)
        ...
        Eaa=sum(sum((penal.*(EMat(Material)-EMat(1)).*xPhys.^(penal-1).*ce)));
        Ebb=penal.*(EMat(Material)-EMat(1)).*xPhys.^(penal-1).*ce;
...
GRAF_ViewDesign=1;
 dd=xPhysAi;
 fname='breal';cc=bi*1000*paso;figure('Name',fname,'Color',[1 1 1]);topUSB_h_graf3
 colormap(gray)
 GRAF_ViewDesign=0;
